...
<div class="row justify-content-center">
    <div class="col-lg-6">
        <br>
        <h1 class="text-center">Edit Data Pelaksana Jumat</h1>
        <form action="/jumatdashboard/{{ $jumats->id }}" method="POST">
            @method('PUT')
            @csrf
            <div class="mb-3">
                <label class="form-label">Tanggal</label>
                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal',$jumats->tanggal)}}" id="tanggal" name="tanggal">
                @error('tanggal')
            ...
            <div class="mb-3">
                <select class="form-control" name="imam_id" aria-label="Default select example">
                    ...
                    @if (old('imam_id',$jumats->imam_id)== $imam->id)
            ...
            <div class="mb-3">
                <select class="form-control" name="muadzin_id" aria-label="Default select example">
                    ...
                    @if (old('muadzin_id',$jumats->muadzin_id)== $muadzin->id)
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('khatib') is-invalid @enderror" id="khatib" name="khatib" value="{{old('khatib',$jumats->khatib)}}">
                @error('khatib')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3">
                <select class="form-control" name="pengurus_id" aria-label="Default select example">
                    ...
                    @if (old('pengurus_id',$jumats->pengurus_id)== $pengurus->id)
            ...
            <div class="btn-group ">
                ...
                <a href="{{url('jumatdashboard')}}" class="btn btn-warning">Kembali</a>
                <div class="offset-md-11"></div>
                <button type="submit" name="submit" class="btn btn-success offset-md-9"><i class="fa fa-edit"></i>  Edit Data</button>
            </div>
            </div>
        </form>
    </div>
