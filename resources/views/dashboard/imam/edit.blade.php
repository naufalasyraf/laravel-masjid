...
<div class="row justify-content-center">
    <div class="col-lg-6">
        <br>
        <h1 class="text-center">Edit Nama Imam Masjid</h1>
        <form action="/imamdashboard/{{ $imams->id }}" method="POST">
            @method('PUT')
            @csrf
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Nama Imam</label>
                <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" value="{{old('nama',$imams->nama)}}">
                @error('nama')
